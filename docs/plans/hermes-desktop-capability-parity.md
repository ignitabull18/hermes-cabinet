...
## Installed Hermes 0.19.0 memory-status contract audit

The installed `hermes memory status` command is a human-oriented diagnostic, not a stable machine-readable interface. Its parser exposes no JSON mode or versioned response schema. The command loads provider plugins, may call provider-specific status and availability hooks, inspects environment-backed configuration, and prints provider-defined configuration values and arbitrary errors. It has no bounded server timeout contract. Because adapting it could expose secret-bearing URLs, environment labels, container metadata, or unbounded provider errors, Cabinet does not invoke it and does not wrap it in a server endpoint.

The safe local observation is limited to the selected provider name in the configured profile and existence of the installed Supermemory plugin manifest. It does not inspect Hermes `.env` files. Provider selection and a detected manifest do not prove the active profile, loaded provider, credential state, runtime availability, or live memory data. This metadata uses `detected_metadata / configured_profile_metadata` evidence, is explicitly non-operational, and earns no Current Live Visibility or Live-Proven credit. Credential state is reported exactly as: **Not inspected — credentials remain owned by Hermes**.
...
| Memory and context | /settings?tab=config:memory | Supported | Management `/api/memory` when configured; local configured-profile metadata only otherwise | [Hermes Memory](/hermes?section=memory) | `visible_read_only` | consequential | Operator | Add a stable authenticated read-only runtime contract before claiming loaded provider, availability, search, or recall. | Installed Agent 0.19.0 CLI/source audit plus non-secret configured-profile metadata; no live runtime proof |
