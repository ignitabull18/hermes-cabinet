...
export const HERMES_CAPABILITY_EVIDENCE_CATALOG: HermesCapabilityEvidenceCatalog = {
  ...
  "agents-subagents": {
    governance: [{
      confirmationBoundary: "One active Kanban run requires a fresh preview and exact typed owner confirmation immediately before termination.",
      stableRequestIdentity: "The action, run, task, start time, active claim fingerprint, and bounded reason derive one opaque request identity.",
      idempotencyBehavior: "Cabinet coalesces duplicate commits and Hermes transactionally compares the active claim; stale or duplicate transitions cannot execute twice.",
      visibleOutcomeEvidence: "Success is shown only after a fresh Hermes read verifies the reclaimed terminal outcome; timeout and failure remain explicit.",
      testedContract: "Hermes Phase 3B governed runtime intervention regression suite",
      proofTimestamp: "2026-07-20T03:30:00Z",
      proofSource: "docs/plans/hermes-governed-runtime-interventions.md",
    }],
    historical: [sourceAudit("agents-subagents", "Installed Desktop agents route audit", "Installed Desktop source exposes agent and subagent status surfaces.")],
  },
