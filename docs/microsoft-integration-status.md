...
_Source contract rechecked: 2026-07-24. Live account verification remains
open where stated._
...
| **OneDrive + SharePoint** | ✅ Desktop-sync folders can be mounted read-only through Connect Knowledge. Microsoft Graph file access is separately available to agents through the Microsoft 365 MCP connector. Cabinet does not project remote Graph files directly into the tree. | Local desktop-sync mount plus community MCP server | Open source | Verify the MCP path against a real work/school tenant; verify local mounts against an installed OneDrive/SharePoint sync root |
